...
{
    [HttpGet]
    public async Task<IActionResult> GetEntries([FromQuery] EntriesQueryParameters query, SortMappingProvider sortMappingProvider, DataShapingService dataShapingService)
    {
        ...
        var sortMappings = sortMappingProvider.GetMappings<EntryDto, Entry>();

        var entriesQuery = dbContext.Entries
            ...
            .Where(e => e.IsArchived == query.IsArchived);
        ...
        {
            Items = dataShapingService.ShapeCollectionData(entries, query.Fields, query.IncludeLinks ? e => CreateLinksForEntry(e.Id, query.Fields, query.IsArchived) : null),
            Page = query.Page,
    ...
    [HttpGet("cursor")]
    public async Task<IActionResult> GetEntriesCursor([FromQuery] EntriesCursorQueryParameters query, SortMappingProvider sortMappingProvider, DataShapingService dataShapingService)
    {
        var userId = await userContext.GetUserIdAsync();
        if (string.IsNullOrWhiteSpace(userId))
            return Unauthorized();
        ...
        if (!dataShapingService.Validate<EntryDto>(query.Fields))
        {
            return Problem(statusCode: StatusCodes.Status400BadRequest,
                detail: $"提供的数据塑形字段无效：{query.Fields}");
        }
        ...
        if (!string.IsNullOrWhiteSpace(query.Cursor))
        {
            ...
            {
                entriesQuery = entriesQuery.Where(e => e.Date < cursor.Date ||
        ...
        var entries = await entriesQuery
            ...
            .Take(query.Limit + 1)//最后1个条目是用来获取游标的
            .Select(EntryQueries.ProjectToDto())
            .ToListAsync();

        var hasNextPage = entries.Count > query.Limit;//仅当条目超限时，才返回
    ...
    public async Task<IActionResult> GetEntry(
        string id, EntryQueryParameters query, DataShapingService dataShapingService)
    {
        var userId = await userContext.GetUserIdAsync();
        if (string.IsNullOrWhiteSpace(userId))
            return Unauthorized();
        ...
        var shapedEntryDto = dataShapingService.ShapeData(entry, query.Fields);

        if (query.IncludeLinks)
        {
            ((IDictionary<string, object?>)shapedEntryDto)[nameof(ILinksResponse.Links)] = CreateLinksForEntry(id, query.Fields, entry.IsArchived);
        }
    ...
    public async Task<ActionResult<EntryDto>> CreateEntry(CreateEntryDto createEntryDto, [FromHeader]AcceptHeaderDto acceptHeader, IValidator<CreateEntryDto> validator)
    {
        ...
        var habit = await dbContext.Habits.FirstOrDefaultAsync(e => e.Id == createEntryDto.HabitId && e.UserId == userId);
        ...
        var entry = createEntryDto.ToEntity(userId, habit);
        ...
        IValidator<CreateEntryBatchDto> validator)
    {
        ...
        var habitIds = createEntryBatchDto.Entries.Select(e => e.HabitId).ToHashSet();

        var existingHabits = await dbContext.Habits
            .Where(e => habitIds.Contains(e.Id) && e.UserId == userId)
            .ToListAsync();
        ...
        var entries = createEntryBatchDto.Entries
            .Select(dto => dto.ToEntity(userId, existingHabits.First(h => h.Id == dto.HabitId))).ToList();
        ...
        IValidator<UpdateEntryDto> validator)
    {
        ...
        var entry = await dbContext.Entries.FirstOrDefaultAsync(e => e.Id == id && e.UserId == userId);
    ...
    private List<LinkDto> CreateLinksForEntries(EntriesQueryParameters parameters, bool hasNextPage, bool hasPreviousPage)
    {
        List<LinkDto> links = [
    ...
    private LinkDto[] CreateLinksForEntry(string id, string? fields, bool isArchived)
    {
        LinkDto[] links = [
