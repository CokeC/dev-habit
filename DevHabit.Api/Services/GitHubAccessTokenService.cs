...
{
    public async Task StoreAsync(string userId, StoreGitHubAccessTokenDto accessTokenDto, CancellationToken cancellationToken = default)
    {
        ...
        if(existingAccessToken is not null)
        {
            ...
            existingAccessToken.ExpiresAtUtc = DateTime.UtcNow.AddDays(accessTokenDto.ExpiresInDays!.Value);
        }
        else
        {
            dbContext.GitHubAccessTokens.Add(new()
            {
                Id = $"gh_{Guid.CreateVersion7()}",
                UserId = userId,
                Token = accessTokenDto.AccessToken,
                CreatedAtUtc = DateTime.UtcNow,
                ExpiresAtUtc = DateTime.UtcNow.AddDays(accessTokenDto.ExpiresInDays!.Value)
            });
    ...
    {
        return await dbContext.GitHubAccessTokens.SingleOrDefaultAsync(e => e.UserId == userId, cancellationToken);
    }
